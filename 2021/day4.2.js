...
let latestSolveIndex = null
let latestSolvedBoardIndex = null
...
for (let ii = 0; ii < boards.length; ii++) {// each board
    ...
    if (latestSolveIndex === null) {
        latestSolveIndex = qbsi
        latestSolvedBoardIndex = ii
    }
    if (qbsi !== null && latestSolveIndex < qbsi) {
        latestSolveIndex = qbsi
        latestSolvedBoardIndex = ii
    }
}
...
const lastNumber = numbers[latestSolveIndex]
const board = boards[latestSolvedBoardIndex]

let restSum = 0

const numberSubset = numbers.slice(0,latestSolveIndex+1)
